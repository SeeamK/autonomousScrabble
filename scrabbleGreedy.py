...
def findAnchorPoints(board):
    # Identify all anchor points (available letters to build off) on the board.
    anchors = set()
    for x in range(15):
        for y in range(15):
            if board[x][y] != "   ":  # Tile exists here
...
class aiAgent:
    def __init__(self, board, dictionary, player):
        self.board = board
        self.dictionary = globals()['dictionary']
        self.player = player

    def simulateMove(self, word, location, direction):
        # calculating the scores of possible words
        pre_simulation_score = self.player.get_score()
        simulated_word = Word(word, location, self.player, direction, self.board.board)
        if simulated_word.check_word() == True:
            simulated_word.calculate_word_score()
            return self.player.get_score() - pre_simulation_score
        return 0


    def generateMoves(self):
        # Check all possible moves AI can make
        potential_moves = []
        filtered_words = filterByRack(self.player.get_rack_arr(), self.dictionary) #find all dictionary words
        anchor_points = findAnchorPoints(self.board.board) #positions where words can connect

        for word in filtered_words:
            for x, y in anchor_points:
                for direction in ["right", "down"]:
                    # Check if word is in board boundaries
                    if direction == "right" and y + len(word) <= 15:
                        simulated_word = Word(word, [x, y], self.player, direction, self.board.board) #simulate placing the word
                        if simulated_word.check_word() == True:
                            potential_moves.append((word, [x, y], direction)) #add if valid word
                    elif direction == "down" and x + len(word) <= 15: #same for down direction placement
                        simulated_word = Word(word, [x, y], self.player, direction, self.board.board)
                        if simulated_word.check_word() == True:
                            potential_moves.append((word, [x, y], direction))
        return potential_moves


    def chooseBestMove(self):
        # Choose the best move from possible moves
        potential_moves = self.generateMoves()
        print(f"Potential moves: { potential_moves }")
        best_move = None
        best_score = -1

        for word, location, direction in potential_moves:
            score = self.simulateMove(word, location, direction)
            if score > best_score:
                best_score = score #track highest score
                best_move = (word, location, direction)

        return best_move



def checkScore(players, target_score=1500):
...
def turn(player, board, bag, dictionary, ai_agent=None):
    # Manage a single turn in the game. If AI is enabled, invoke the agent's decision-making
    ...
    if (skipped_turns < 6) or (player.rack.get_rack_length() == 0 and bag.get_remaining_tiles() == 0):
        ...
        if ai_agent and player.name == "AI":
            print("beofre choose move")
            move = ai_agent.chooseBestMove()
        ...
        # Ai agent
        ai_agent = aiAgent(board, dictionary, player)

        # Call the turn function
        turn(player, board, bag, dictionary, ai_agent=ai_agent)
    else:
        end_game()
...
def start_game():
    ...
    turn(current_player, board, dictionary, bag)
