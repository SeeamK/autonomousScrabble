...
class Word:
    ...
    def check_word(self):
        ...
        if self.word != "":
            ...
            #             if self.board.board[self.location[0]][self.location[1]+i][1] == " " or self.board.board[self.location[0]][self.location[1]+i] == "TLS" or self.board.board[self.location[0]][self.location[1]+i] == "TWS" or self.board.board[self.location[0]][self.location[1]+i] == "DLS" or self.board.board[self.location[0]][self.location[1]+i] == "DWS" or self.board.board[self.location[0]][self.location[1]+i][1] == "*":
            #                 current_board_ltr += " "
            #             else:
            #                 current_board_ltr += self.board.board[self.location[0]][self.location[1]+i][1]
            #     elif self.direction == "down":
            #         print("in down ai")
            #         for i in range(len(self.word)):
            #             if self.board.board[self.location[0]+i][self.location[1]] == "   " or self.board.board[self.location[0]+i][self.location[1]] == "TLS" or self.board.board[self.location[0]+i][self.location[1]] == "TWS" or self.board.board[self.location[0]+i][self.location[1]] == "DLS" or self.board.board[self.location[0]+i][self.location[1]] == "DWS" or self.board.board[self.location[0]+i][self.location[1]] == " * ":
            #                 current_board_ltr += " "
            #             else:
            #                 current_board_ltr += self.board.board[self.location[0]+i][self.location[1]][1]
            #     else:
...
class Node:
    ...
    def get_legal_moves(self):
        board, player_rack = self.game_state["board"], self.game_state["rack"]
        anchor_points = findAnchorPoints(board)  
        valid_words = filterByRack(player_rack, dictionary) #words thata re formable from rack
        moves = []
        for word in valid_words:
            for x, y in anchor_points:
                for direction in ["right", "down"]:
                    if direction == "right" and y + len(word) <= 15:
                        moves.append((word, [x, y], direction))
                    elif direction == "down" and x + len(word) <= 15:
                        moves.append((word, [x, y], direction))
        return moves
    ...
    def best_child(self, exploration_weight=1.41):
        return max(
            self.children,
            key=lambda child: (child.win_score / child.visits) +
                              exploration_weight * math.sqrt(math.log(self.visits) / child.visits)
        )

#maybe helps: https://www.geeksforgeeks.org/upper-confidence-bound-algorithm-in-reinforcement-learning/
class MCTS:
    def __init__(self, root_state):
        self.root = Node(root_state)

    def run(self, iterations=1000):
        #perform 1k iterations
        for _ in range(iterations):
            leaf = self.selection(self.root)  #go until find the leaf
            if not leaf.is_fully_expanded():
                child = self.expansion(leaf)  #expand it
                simulation_result = self.simulation(child) #random
                self.backpropagation(child, simulation_result)#update tree

    def selection(self, node):
        #traverse until finding the best leaf
        while node.children:
            node = node.best_child()
        return node

    def expansion(self, node):
        #expand the node by adding an untried move child
        untried_moves = [
            move for move in node.get_legal_moves()
            if move not in [child.move for child in node.children]
        ]
        move = random.choice(untried_moves)# get an untried one
        new_game_state = self.simulate_move(node.game_state, move)
        child_node = Node(new_game_state, move, parent=node)
    ...
    def simulation(self, node):
        ...
        while not self.is_terminal_state(current_state):
            possible_moves = node.get_legal_moves()
            if possible_moves:
                move = random.choice(possible_moves)  #random move
    ...
    def simulateMove(self, state, move):
        #ill fix it 
        #this might need to change cause its not working its placing it in dumb spots doesnt make sense
        simulated_state = copy.deepcopy(state)  
        board = simulated_state["board"]
        player = simulated_state["player"]
        word, location, direction = move

        board.place_word(word, location, direction, player)  
        player.rack.replenish_rack() 
        
        simulated_state["board"] = board
        simulated_state["rack"] = player.rack.get_rack_arr()
        simulated_state["score"] = player.get_score()
        simulated_state["tiles_left"] -= len(word)

        return simulated_state

    def isTerminal(self, state):
        #did the game end
        return state["tiles_left"] <= 0 or state["score"] >= 100
...
def filterByRack(rack, dictionary):
    # Return words from the dictionary that can be formed using the given rack.
    rack_letters = "".join(tile.get_letter() for tile in rack)
    valid_words = []
    for word in dictionary:
        temp_rack = list(rack_letters)  # Create a mutable copy
        valid = True
        for letter in word:
...
def findAnchorPoints(board):
    # Identify all anchor points (cells adjacent to existing tiles) on the board
    boardArray = board.board_array()
    anchors = set()
    for x in range(15):
        for y in range(15):
            if boardArray[x][y] != "   ":  # Tile exists here
                for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:  # Check neighbors
                    nx, ny = x + dx, y + dy
                    if 0 <= nx < 15 and 0 <= ny < 15 and boardArray[nx][ny] == "   ":
                        anchors.add((nx, ny))
    return anchors

def checkScore(players, target_score=100):
    for player in players:
        if player.get_score() >= target_score:
            print(f"Player {player.get_name()} has won with score greater than {target_score}!")
            end_game()
    


def turn(player, board, bag, dictionary):
    # Manage a single turn in the game
    global round_number, players, skipped_turns

    checkScore(players=players)
    ...
    if (skipped_turns < 6) or (player.rack.get_rack_length() == 0 and bag.get_remaining_tiles() == 0):
        ...
        if player.name == "AI":
            # Set up the root game state for MCTS
            root_state = {
                "board": board,
                "rack": player.get_rack_arr(),
            ...
            mcts.run(iterations=1000)  # Adjust the number of iterations as needed
            best_move = mcts.root.best_child().move

            if best_move:
                word, location, direction = best_move
                print(f"AI plays {word} at {location} {direction}")
                board.place_word(word, location, direction, player)
                wordObj = Word(word, location, player, direction, board.board_array())
                wordObj.calculate_word_score()
                skipped_turns = 0
            else:
                print("AI skips the turn.")
                skipped_turns += 1
        else:
            # Original logic for human player
        ...
        # Recursively call the function for the next turn
        turn(next_player, board, bag, dictionary)
    else:
        end_game()
...
def start_game():
    ...
    global round_number, players, skipped_turns, dictionary
    board = Board()
    bag = Bag()
    ...
    turn(current_player, board, bag, dictionary)
